...
-----------------------------  OpenFile  ---------------------------------
  ...
  --GetANewThread: UserProgram: initialize, fork
  --function: StartUserProcess : starting up a user lever prog
  --StartUserProcess : allocate a new PCB (with GetANewProcess) and connect it with the thread
  --initialize the myThread field in the PCB and the myProcess field in the current thread.
  --open the executable file
  --OpenFile.LoadExecutable: create the logical address space and read the executable into it
  --LoadExecutable returns the entry point, which you might call initPC.
  --close executable files
  --InitUserStackTop: compute the initial value for the user-level stack
  ...
  function StartUserProcess()
    ...
    fileName = fileManager.Open("Test Program one")
    if fileName == null
      print("************\n Could not open file \n****************")
    endIf
