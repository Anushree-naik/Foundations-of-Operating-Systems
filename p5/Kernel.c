code Kernel
  ...
  -- This function is used to initialize and fork the thread object
  -- The new thread  runs the StartUserProcess
  -- StartUserProcess does the work of starting up the user level process
  -- InitFirstProcess supplies zero as an argument to StartUserProcess 
  -- and it returns after forking the new thread.

  function InitFirstProcess()

  var 
    --Declaring new thread pointer
    threadpt: ptr to Thread

    -- Initializing. This fucntion will return new thread.
    threadpt = threadManager.GetANewThread()
    
    threadpt.name = "UserProgram"
    
    --Creating a new thread which will run the StartUserProcess function.
    threadpt.Fork(StartUserProcess,0)
  ...
  function StartUserProcess()
    ...
    --This function allocates new PCB and connects it with the thread
    -- StartUserProcess function jumps from kernel level program to user level 
    -- program. 
    -- It does not return
    -- Inorder to return, we have to execute syscall.

    var 
    

      -- Declaring new pointer to open file
      fileName: ptr to OpenFile

      -- Declaring new pointer to PCB
      pcb: ptr to ProcessControlBlock
      initPC: int

      -- Declaring pointer to Address space
      addrSpace: ptr to AddrSpace
      
      -- Declaring InitUserStackTop and initSystemStackTop
    ...
    -- Opening the TestProgram1 file.

    fileName = fileManager.Open("TestProgram1")
    if fileName == null
      print("************\n Could not open file \n****************")
    endIf


    --This function will create a virtual address space to load a.out file in 
    -- it
    
    initPC = fileName.LoadExecutable(addrSpace)
    ...
    -- Initializing the value of user stack to the logical address space
    InitUserStackTop = addrSpace.numberOfPages * PAGE_SIZE
    
    -- This is the initial value of the system stack
    initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]


    -- This converts the current thread into the user level thread
    oldIntStat = SetInterruptsTo(DISABLED)
    --This sets the page table register for the current logical address space
    addrSpace.SetToThisPageTable()

    -- If current thread is a user thread, "BecomeUserThread" method changes
    -- the mode bit to perform the atomic jump. 
    -- Jumps to user lever thread routine
...
-----------------------------  ThreadFinish  ---------------------------------
  ...
  function ThreadFinish ()
    ...
    -- we cant actually do the clean up.  So we just make a note
...
-----------------------------  MutexHoare ---------------------------------
  ...
  behavior MutexHoare
    ...
    --         on the mutex.
       ...
       ---------- MutexHoare . GiveUpLock -----------  

       method GiveUpLock(th: ptr to Thread)
       
        heldBy = th
       
       endMethod

       -----------  MutexHoare . IsHeldByCurrentThread  -----------
...
-----------------------------  ConditionHoare  ---------------------------------
  ...
  behavior ConditionHoare
    ...
    --         Each condition must be initialized.
      ...
      method Wait (mutex: ptr to MutexHoare)
          var
            oldIntStat: int
          if ! mutex.IsHeldByCurrentThread ()
            FatalError ("Attempt to wait on condition when mutex is not held")
      ...
      method Signal (mutex: ptr to MutexHoare)
          var
            oldIntStat: int
            t: ptr to Thread
          if ! mutex.IsHeldByCurrentThread ()
          ...
          if t
            ...
            -- to call GiveUpLock so to implement the Hoare semantics
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  -- This behavior gives Init, Print, GetANewThread, FreeThread methods
  -- ThreadManager checks when the process starts running, a kernel will 
  -- obtain a new thread object for it and when the process dies, thread 
  -- object must be retured to the pool of free threads. So it can be 
  -- recycled for the reuse.
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          var 
            ...
            -- Initializing new array of threads.
            threadTable = new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread}
            -- Initializing free list and place all Threads in the 
            -- threadTable array on the freeList during initialization.
            freeList = new List[Thread]
            
            -- Initializing Condition variable and mutex
            ...
            threadTable[0].Init("0")
            threadTable[1].Init("1")
            threadTable[2].Init("2")
            threadTable[3].Init("3")
            threadTable[4].Init("4")
            threadTable[5].Init("5")
            threadTable[6].Init("6")
            threadTable[7].Init("7")
            threadTable[8].Init("8")
            threadTable[9].Init("9")


            for i = 0 to MAX_NUMBER_OF_PROCESSES-1
            -- Initializing  the threads in the array as unused
              threadTable[i].status = UNUSED
            -- This will add the treads to the end of the freeList  
      ...
      method GetANewThread () returns ptr to Thread
        ...
        var
          threadpt : ptr to Thread
        -- Putting the lock on threadManagerLock to acheive the concurrency.
        threadManagerLock.Lock()

        -- While loop to check if the list is empty or not. 
        -- If empty then it waits. This will keep executing until 
        -- a thread is available and is in the free list
        while
          freeList.IsEmpty()
          aThreadBecameFree.Wait(& threadManagerLock)
        endWhile
        
        -- when a free thread is allocated to some process, 
        -- remove it from freeList and make its status to JUST_CREATED
          threadpt = freeList.Remove()
          threadpt.status = JUST_CREATED

        -- Unlock the threadManagerLock and return the thread pointer
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- Lock the threadManagerLock 
        -- to achieve the concurrency
        threadManagerLock.Lock()

        -- make the status of the thread which is free to unused
        th.status = UNUSED
        
        -- add the thread to the end of the free list
        freeList.AddToEnd(th)
        -- Signal that a thread is available and can be used for the next task
        aThreadBecameFree.Signal(& threadManagerLock)

        -- unlock the threadManagerLock
        threadManagerLock.Unlock()
...
-- This block implements following : Init, Print, GetANewThread, FreeThread 
-- Purpose of this block to implemet the hoare semantics and provide the 
-- execution in that manner where the starvation wont be an issue

  behavior ThreadManagerHoare

      ----------  ThreadManagerHoare . Init  ----------

      method Init ()
        --
        -- This method is called once at kernel startup time to initialize
        -- the one and only "ThreadManagerHoare" object.
        
        -- Initializing variables 
          var 
            i: int

            -- Initializing array elements of the array threadTable
            threadTable = new array of Thread {MAX_NUMBER_OF_PROCESSES of new Thread}
            freeList = new List[Thread]
            
            -- Initializing Condition variable and mutex
            aThreadBecameFree = new ConditionHoare
            threadManagerLock = new MutexHoare
            ...
            for i = 0 to MAX_NUMBER_OF_PROCESSES-1
            -- keep the status of the threads present in the array as unused
            -- since they are not allocated to some task  
              threadTable[i].status = UNUSED
      ...
      ----------  ThreadManagerHoare . GetANewThread  ----------
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  -- This behavior gives Init, Print, PrintShort, GetANewProcess, FreeProcess methods
  -- All processes will be preallocated in an array called processTable, 
  -- which will be managed by the ProcessManager object
  -- There are resources which must be allocated. 
  -- This is the purpose of the monitor class called ProcessManager.
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        var 
          ...
          -- Initializing array processTable
          processTable = new array of ProcessControlBlock {MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock}

          -- Initializing free list which will have all the processes that are free
          ...
          for i = 0 to MAX_NUMBER_OF_PROCESSES-1
          -- At start-up time, all ProcessControlBlocks are initially FREE     
          -- These objects will be allocated and when the user-level process dies, 
          -- the corresponding ProcessControlBlock will become FREE once again.
            processTable[i].status = FREE
          
          -- Add the free processes to the end of the free list. 
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- Initializing variable   
        var 
          processpt : ptr to ProcessControlBlock

          -- Locking processManagerLock
          processManagerLock.Lock()


          nextPid = nextPid + 1

          -- This loop will check if the free list is empty, 
          -- if yes then it sends the message as to wait until the process is free
          while freeList.IsEmpty()
            aProcessBecameFree.Wait( & processManagerLock) 
          endWhile
          
          -- When the process gets free, remove it from the freeList and 
          -- keep its status as ACTIVE
          processpt = freeList.Remove()
          processpt.status = ACTIVE

          -- If the process status is ZOMBIE, 
          -- Signal the condition variable aProcessDied
          if processpt.status == ZOMBIE
            aProcessDied.Signal(& processManagerLock)
          endIf
          
          -- Unlock processManagerLock and return process pointer
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- Lock the processManagerLock
          processManagerLock.Lock()

        -- When the status of the process is free, 
        -- Add to the end of the free list  
          p.status = FREE
          freeList.AddToEnd(p)

        -- Signal indicating that the process is now Free
        -- and can be used/reused.
          aProcessBecameFree.Signal(& processManagerLock)

        -- Unlock processManagerLock  
          processManagerLock.Unlock()
        endMethod
...
-- This block contains following methods: Init, Print, GetAFrame, GetNewFrames, ReturnAllFrames   
  ...
  behavior FrameManager
      ...
      -- Each frame is a resource and Frame Manager class can either satisfy the request for the resources
      -- or it can tell the requesting process to wait until all the required number of frames are available

      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
      -- Initializing variables
          var Frames, i , frameAddr , numOfWaitingThreads: int
      
      -- Locking the frameManagerLock    
          frameManagerLock.Lock()

      -- This loop checks if the numOfWaitingThreads are greater than 1, 
      -- then Increment the count and tell other threads to wait.    
          
          while (numOfWaitingThreads > 1)
            numOfWaitingThreads = numOfWaitingThreads + 1
            otherThreads.Wait(& frameManagerLock)
          endWhile

      -- This loop checks if the number of free frames are less than 
      -- the number of frames required, if so wait.     
          while numberFreeFrames < numFramesNeeded
            leaderThread.Wait(& frameManagerLock)
          endWhile


      -- check the frames that are in use and set the bit to one
      -- Calculate the physical address
      -- set the frame address      
          
          for i = 0 to numFramesNeeded - 1
            
            Frames = framesInUse.FindZeroAndSet()
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (Frames * PAGE_SIZE)
            aPageTable.SetFrameAddr(i ,frameAddr)
          
          endFor  

      -- Decrement the count of number of waiting threads    
      -- and signal to the other threads
          numOfWaitingThreads = numOfWaitingThreads - 1
          otherThreads.Signal(& frameManagerLock)

      -- Update the count of number of free frames by subtracting the 
      -- count of the frames that are needed
          numberFreeFrames = numberFreeFrames - numFramesNeeded
          aPageTable.numberOfPages = numFramesNeeded

      -- Unlock the frameManagerLock
          frameManagerLock.Unlock()
      ...
      -- This method will return all the frames that are free
      -- It will also need to adjust the number of free frames and 
      -- “notify” any waiting threads that more frames have become available.


      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
      
      -- Initialize the variables    
          var numFramesReturned, bitNumber, i, frameAddr , numOfWaitingThreads : int
      
      -- Lock the frameManagerLock    
          frameManagerLock.Lock ()

          numFramesReturned = aPageTable.numberOfPages

      -- This loop will Calculate the bit number     
      ...
      -- Increment the count of number of free frames and 
      -- signal the leader thread   
...
-----------------------------  AddrSpace  ---------------------------------
  ...
  behavior AddrSpace
      ...
      method SetValid (entry: int)
        --
        -- Set the selected page table entrys "valid" bit to 1.
        --
      ...
      method ClearDirty (entry: int)
        --
        -- Clear the selected page table entrys "dirty" bit.
        --
          pageTable[entry] = pageTable[entry] & ! 0x00000008
        endMethod
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  -- after the current process and its thread is terminated,
  -- this function prints that the "Exit" is invoked
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  -- after causing an immediate shutdown,  
  -- this function prints that "Shutdown" is invoked
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  -- after yeilding the CPU to another process,
  -- this function prints " Yield " is invoked
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
    ...
    -- after creating a new process which is a copy of the new process,
    -- this function prints "Fork " is invoked
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
    ...
    -- after indicating the caller to wait until the process with given PID is 
    -- terminated, by executing exit sys call, 
    -- this function prints "Join" is invoked with the process Id
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
   ...
   -- This function starts executing the executable files 
   -- These files are passed a parameter to this function
   -- It is read into the memory, over writing the entire address space of 
   -- the current process. OS will be executing new process.
   -- This function will not return. 
   -- if there are any problems, it will return -1

    var 
    newAddrSpace : AddrSpace = new AddrSpace
    stringBuffer : array [MAX_STRING_SIZE] of char
    fileName : ptr to OpenFile
    initPC : int
    ...
    -- this translates the virtual address space to the physical address space.

    i = currentThread.myProcess.addrSpace.GetStringFromVirtual (& stringBuffer, filename asInteger, MAX_STRING_SIZE) 
    ...
    -- Open the file
    -- If null, print "could not open"
    fileName = fileManager.Open(&stringBuffer)
    if fileName == null
      print("******** \n Could not open file \n **********")
      return -1
    endIf
    ...
    initPC = fileName.LoadExecutable(&newAddrSpace)
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      -- After creating the new file on the disk,
      -- this function prints "Create" is invoked

      var 
        i : int
        stringBuffer : array [MAX_STRING_SIZE] of char
        print("\n Handle_Sys_Create invoked \n")
        print("Virtual address of the file : ")
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      -- after opening the already existing files
      -- this function prints "Open" invoked

      var
        i: int
        stringBuffer: array [MAX_STRING_SIZE] of char
        print("\n Handle_Sys_Open invoked \n")
        print("Virtual address of the file name: ")
        printHex(filename asInteger)
        nl()
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      --after reading the bytes from the file and placing them into the memory,
      -- this function prints "Read" is invoked
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      -- after passing the pointer to the area of memory and the count of 
      -- numbers of bytes to transfer, write function writes those many 
      -- bytes from the memmory to the current position in the file.
      -- This function prints "write " invoked
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
      ...
      -- after setting the current position in the given file, 
      -- to the given value and returning the new position,
      -- this function prints "Seek" invoked
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
      ...
      -- after closing the file described by the fileDesc,
      -- this function prints "close " invoked
